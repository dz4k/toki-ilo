...
import { makeContext, functionContext } from "./stdlib.js"
 ...
 */
const run = async (ast, functions = {}) => {
    const context = makeContext(functions)
    ...
    try {
        ...
        case "la statement": {
            ...
            case "funcall statement": {
                ...
                const [nameNode, ...args] = lhs.children
                const funcName = mangleFunctionName(nameNode.data, args)
                const argNames = args.map(arg => arg.data)
                ctx[functionContext][funcName] = (...args) => {
                    const fctx = deriveContext(ctx)
                    for (let i = 0; i < argNames.length; i++) {
                        ctx[argNames[i].data] = args[i]
                    }
                    runNode(rhs, fctx)
                }
            }
        ...
        console.error(node, ctx)
        throw e
    }
}

const deriveContext = (ctx) => Object.assign(Object.create(ctx), {
        [functionContext]: Object.create(ctx[functionContext])
    })
